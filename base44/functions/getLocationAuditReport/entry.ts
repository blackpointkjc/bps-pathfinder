...
Deno.serve(async req => {
  try {
    ...
    const [historyRows, alertRows, timeEntries] = await Promise.all([
    ...
    return Response.json({ history, geofenceAlerts, entries, date, timeZone: 'America/New_York', generatedAt: new Date().toISOString() });
  } catch (error) {
