  ...
  try {
      ...
      const units = [...newestByEmail.values()]
        ...
        .map((active: any) => {
          ...
          return {
            ...
            coarse_latitude: !hasGps && Number.isFinite(gpsTs) && gpsTs >= gpsFreshCutoff && hasValidCoordinates(active.latitude, active.longitude) ? Number(active.latitude) : null,
            coarse_longitude: !hasGps && Number.isFinite(gpsTs) && gpsTs >= gpsFreshCutoff && hasValidCoordinates(active.latitude, active.longitude) ? Number(active.longitude) : null,
            coarse_accuracy: !hasGps && Number.isFinite(gpsTs) && gpsTs >= gpsFreshCutoff && Number.isFinite(accuracy) ? accuracy : null,
            gps_pending: !hasGps,
    ...
    for (const [email, active] of newestActiveByEmail.entries()) {
      ...
      units.push({
        ...
        coarse_latitude: !hasReliableGps && Number.isFinite(gpsTs) && gpsTs >= gpsFreshCutoff && hasValidCoordinates(active.latitude, active.longitude) ? Number(active.latitude) : null,
        coarse_longitude: !hasReliableGps && Number.isFinite(gpsTs) && gpsTs >= gpsFreshCutoff && hasValidCoordinates(active.latitude, active.longitude) ? Number(active.longitude) : null,
        coarse_accuracy: !hasReliableGps && Number.isFinite(gpsTs) && gpsTs >= gpsFreshCutoff && Number.isFinite(accuracy) ? accuracy : null,
    ...
    const onDutyUsers = (users || [])
      ...
      .map((user: any) => {
        ...
        return {
          ...
          coarse_latitude: !hasFreshGps && Number.isFinite(gpsTs) && gpsTs >= gpsFreshCutoff && hasValidCoordinates(active?.latitude, active?.longitude) ? Number(active.latitude) : null,
          coarse_longitude: !hasFreshGps && Number.isFinite(gpsTs) && gpsTs >= gpsFreshCutoff && hasValidCoordinates(active?.latitude, active?.longitude) ? Number(active.longitude) : null,
          coarse_accuracy: !hasFreshGps && Number.isFinite(gpsTs) && gpsTs >= gpsFreshCutoff && Number.isFinite(accuracy) ? accuracy : null,
