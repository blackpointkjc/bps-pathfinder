...
Deno.serve(async (req) => {
  try {
    ...
    const allExceptCaller = body.all_except_caller === true;
    if (!reason) return Response.json({ error: 'A force sign-out reason is required' }, { status: 400 });

    if (allExceptCaller) {
      const [users, activeOfficers, units] = await Promise.all([
        ...
        base44.asServiceRole.entities.Unit.list(undefined, 1000).catch(() => []),
      ]);
      const now = new Date();
      const issuedAt = now.toISOString();
      const expiresAt = new Date(now.getTime() + 5 * 60 * 1000).toISOString();
      const actorName = [caller.rank, caller.last_name].filter(Boolean).join(' ') || caller.full_name || caller.email;
      const activeUserIds = new Set((units || [])
        .filter((unit: any) => lower(unit.status) !== 'out of service')
        .map((unit: any) => String(unit.user_id || ''))
        .filter(Boolean));
      const activeEmails = new Set([
        ...(activeOfficers || []).filter((entry: any) => entry.session_active !== false).map((entry: any) => lower(entry.officer_email)),
        ...(units || []).filter((unit: any) => lower(unit.status) !== 'out of service').map((unit: any) => lower(unit.user_email)),
      ].filter(Boolean));
      const targets = (users || []).filter((target: any) =>
        target?.id !== caller.id
        && target?.email
        && (activeUserIds.has(String(target.id)) || activeEmails.has(lower(target.email)))
      );

      let retiredLiveSessions = 0;
      let retiredUnits = 0;
      for (const target of targets) {
        ...
        await Promise.all((earlier || []).map((record: any) =>
          base44.asServiceRole.entities.UserSessionControl.update(record.id, { active: false }).catch(() => null)
        ));

        await base44.asServiceRole.entities.UserSessionControl.create({
          user_id: target.id,
        ...
        retiredLiveSessions += targetSessions.length;
        await Promise.all(targetSessions.map((session: any) =>
          base44.asServiceRole.entities.ActiveOfficer.update(session.id, {
        ...
        const targetUnits = (units || []).filter((unit: any) =>
          unit.user_id === target.id || lower(unit.user_email) === lower(target.email)
        );
        retiredUnits += targetUnits.length;
        await Promise.all(targetUnits.map((unit: any) =>
      ...
      await base44.asServiceRole.entities.AuditLog.create({
        entity_type: 'UserSessionControl',
        entity_id: caller.id,
        action: 'status_change',
        actor_id: caller.id,
        actor_name: actorName,
        before_value: JSON.stringify({ active_units: targets.length }),
        after_value: JSON.stringify({ force_signed_out: targets.length, excluded_caller: caller.id }),
        field_changed: 'authenticated_session',
        timestamp: issuedAt,
        description: `Forced sign-out issued for all active units except the initiating administrator. Reason: ${reason}`,
      }).catch(() => null);

      return Response.json({
        success: true,
        all_except_caller: true,
        signed_out_users: targets.length,
        retired_live_sessions: retiredLiveSessions,
