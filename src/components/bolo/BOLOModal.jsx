...
function FormView({ data, onChange }) {
  ...
  return <div className="space-y-4 font-mono">
    ...
    <BoloAttachmentsEditor attachments={data.attachments || []} onChange={value => set('attachments', value)} />
...
export default function BOLOModal({ mode, bolo, user, onClose, onSaved, onEdit }) {
  const draftKey = useMemo(() => `bps-bolo-draft:${user?.id || user?.email || 'user'}:${mode}:${bolo?.id || 'new'}`, [user?.id, user?.email, mode, bolo?.id]);
  const initial = useMemo(() => {
    const base = { ...bolo, parties: bolo?.parties?.length ? bolo.parties : legacyParty(bolo), vehicles: bolo?.vehicles?.length ? bolo.vehicles : legacyVehicle(bolo), photo_urls: bolo?.photo_urls || [], attachments: bolo?.attachments || [] };
