...
const BOLO_LABELS = {
  wanted_person: ['FUGITIVE FILE', 'FUGITIVE FILES'],
...
function boloGroupSegments(bolos = []) {
  ...
  for (const bolo of bolos) {
    ...
    // Matching priority/type is similar criteria. Underlying BOLO files stay
    // separate; only the global banner presentation is merged.
    const key = `${priority}|${type}`;
    const group = groups.get(key) || { priority, type, items: [] };
  ...
  return [...groups.values()]
    ...
    .map(group => {
      ...
      const [singular, plural] = BOLO_LABELS[group.type] || ['BOLO FILE', 'BOLO FILES'];
      ...
      const detail = count === 1
        ...
        : locations.length === 1
          ...
          : locations.length > 1
            ...
            : '';
      return {
        key: `bolo:${group.priority}:${group.type}`,
        kind: 'bolo',
