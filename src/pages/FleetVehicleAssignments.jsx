...
import { Car, ChevronLeft, ChevronRight, Users, Wrench, CheckCircle2 } from 'lucide-react';
    ...
    <div className="mx-auto max-w-[1500px] space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-[360px_1fr]">
        ...
        <div className="overflow-x-auto rounded border border-slate-800 bg-slate-900">
          ...
          <div className="grid min-w-[980px] grid-cols-7 min-h-[520px]">{dates.map(d=><div key={d} className="space-y-2 border-r border-slate-800 p-2 last:border-r-0">{assignments.filter(a=>a.assignment_date===d && (isAdmin || a.primary_officer_email===user?.email || a.partner_officer_email===user?.email)).map(a=><div key={a.id} className="rounded border border-amber-700/50 bg-amber-950/20 p-2 text-[10px]"><div className="font-black text-amber-300">{a.vehicle_label}</div><div>{a.start_time}-{a.end_time}</div><div>{a.primary_officer_name}</div>{a.partner_officer_name && <div className="flex items-center gap-1 text-blue-300"><Users className="h-3 w-3"/>{a.partner_officer_name}</div>}<div className="truncate text-slate-500">{a.location}</div></div>)}</div>)}</div>
        </div>
      </div>
    </div>
  </div>;
