...
const FIELD_RANKS=['Captain','Lieutenant','First Sergeant','Sergeant','Corporal','Senior officer','Officer'];
const rankOrder=r=>{const i=FIELD_RANKS.indexOf(r);return i<0?99:i};
...
export default function VAContactSheet(){
 ...
 const operational=useMemo(()=>users.filter(u=>!u.termination_date&&FIELD_RANKS.includes(u.rank)&&has(u,'cad_access')&&has(u,'officer')).sort((a,b)=>rankOrder(a.rank)-rankOrder(b.rank)||String(a.last_name||'').localeCompare(String(b.last_name||''))),[users]);
 const staff=useMemo(()=>users.filter(u=>!u.termination_date&&!operational.some(o=>o.id===u.id)&&(has(u,'hr')||has(u,'support_staff')||has(u,'support')||has(u,'company_staff')||['human resources','support staff','company staff'].includes(String(u.rank||'').toLowerCase()))).sort((a,b)=>String(a.last_name||'').localeCompare(String(b.last_name||''))),[users,operational]);
 ...
 const Person=({u})=><div className="grid gap-2 border-t border-slate-800 p-3 first:border-t-0 md:grid-cols-[1.2fr_1fr_1.4fr_1fr_.7fr] md:items-center"><div><div className="font-bold text-white">{u.last_name}, {u.first_name}</div><div className="text-[10px] text-slate-500">{u.division||'Unassigned Division'}</div></div><div><Badge variant="outline" className="border-slate-700 text-slate-300">{u.rank||'Company Staff'}</Badge></div><a href={`mailto:${u.email}`} className="flex min-w-0 items-center gap-1 text-xs text-blue-400"><Mail className="h-3 w-3 shrink-0"/><span className="truncate">{u.email}</span></a><div>{u.mobile_phone?<a href={`tel:${u.mobile_phone}`} className="flex items-center gap-1 text-xs text-blue-400"><Phone className="h-3 w-3"/>{u.mobile_phone}</a>:<span className="text-xs text-slate-500">—</span>}</div><div className="text-xs text-slate-400">{u.unit_number?`Officer #${u.unit_number}`:'—'}</div></div>;
