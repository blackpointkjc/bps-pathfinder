  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-[#050a11] font-sans text-white">
      ...
      <div className="flex flex-none flex-wrap items-center gap-2 border-b border-[#24354c] bg-[#0b1320] px-3 py-2">
        <button type="button" aria-pressed={view === 'active'} onClick={() => { setView('active'); setTypeFilter('all'); }} className={`flex items-center gap-1.5 rounded border px-3 py-1 text-[9px] font-black ${view === 'active' ? 'border-green-500/60 bg-green-950/40 text-green-300' : 'border-slate-700 text-slate-500'}`}><Radio className="h-3 w-3" />ACTIVE ({activeCount})</button>
        <button type="button" aria-pressed={view === 'drafts'} onClick={() => { setView('drafts'); setTypeFilter('all'); }} className={`flex items-center gap-1.5 rounded border px-3 py-1 text-[9px] font-black ${view === 'drafts' ? 'border-amber-500/60 bg-amber-950/40 text-amber-200' : 'border-slate-700 text-slate-500'}`}><Edit2 className="h-3 w-3" />DRAFTS ({draftCount})</button>
        <button type="button" aria-pressed={view === 'history'} onClick={() => { setView('history'); setTypeFilter('all'); }} className={`flex items-center gap-1.5 rounded border px-3 py-1 text-[9px] font-black ${view === 'history' ? 'border-blue-500/60 bg-blue-950/40 text-blue-300' : 'border-slate-700 text-slate-500'}`}><History className="h-3 w-3" />PAST / RESOLVED ({historyCount})</button>
        <div className="h-5 w-px bg-slate-700" />
        <button type="button" aria-pressed={typeFilter === 'all'} onClick={() => setTypeFilter('all')} className={`rounded px-2 py-1 text-[9px] ${typeFilter === 'all' ? 'bg-slate-700 text-white' : 'text-slate-500'}`}>ALL TYPES</button>
        {Object.entries(TYPE_CONFIG).map(([key,cfg]) => <button type="button" aria-pressed={typeFilter === key} key={key} onClick={() => setTypeFilter(typeFilter === key ? 'all' : key)} className={`rounded px-2 py-1 text-[9px] ${typeFilter === key ? 'bg-slate-700 text-white' : 'text-slate-500 hover:text-slate-300'}`}>{cfg.label}</button>)}
      </div>

      <div className="hidden flex-none grid-cols-12 border-b border-[#24354c] bg-[#111a29] px-3 py-1.5 text-[8px] font-bold tracking-widest text-slate-500 md:grid">
