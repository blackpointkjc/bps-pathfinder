  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-[#050a11] font-mono text-white">
      ...
      <div className="flex flex-none flex-wrap items-center gap-2 border-b border-[#24354c] bg-[#0b1320] px-3 py-2">
        ...
        {Object.entries(TYPE_CONFIG).map(([key,cfg]) => <button key={key} onClick={() => setTypeFilter(typeFilter === key ? 'all' : key)} className={`rounded px-2 py-1 text-[9px] ${typeFilter === key ? 'bg-slate-700 text-white' : 'text-slate-500 hover:text-slate-300'}`}>{cfg.label}</button>)}
      </div>
