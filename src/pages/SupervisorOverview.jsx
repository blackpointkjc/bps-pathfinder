    ...
    <div className="min-h-[calc(100vh-190px)] bg-[#070d17] p-4 text-white md:p-6">
      <div className="mx-auto max-w-[1500px] space-y-5">
        ...
        <div className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]">
          ...
          <section className="rounded-2xl border border-slate-800 bg-[#0a1421] p-5"><div className="text-xs font-black uppercase tracking-[.16em] text-blue-300">Pending Actions</div><h3 className="mt-1 text-xl font-black">Supervisor work queue</h3><div className="mt-4 space-y-2">{taskQueue.map(item => <div key={item.id} className="rounded-xl border border-slate-800 bg-[#0d1a2a] p-3"><div className="text-sm font-black">{item.title}</div><div className="text-xs font-bold text-blue-200">{item.person}</div><div className="mt-1 text-xs text-slate-500">{item.detail}</div><button type="button" onClick={()=>finishTask.mutate(item)} disabled={finishTask.isPending} className="mt-2 inline-flex items-center gap-1.5 rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-3 py-1.5 text-[10px] font-black text-emerald-200 hover:bg-emerald-500/20 disabled:opacity-50"><ShieldCheck className="h-3.5 w-3.5"/>{finishTask.isPending && finishTask.variables?.id===item.id ? 'FINISHING…' : 'MARK FINISHED'}</button></div>)}{!taskQueue.length && <div className="rounded-xl border border-dashed border-slate-700 p-5 text-center text-sm text-slate-500">No personnel actions are waiting.</div>}</div></section>
        </div>
