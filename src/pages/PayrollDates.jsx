...
export default function PayrollDates({ readOnly = false }) {
  ...
  return <div className="min-h-screen p-4 md:p-8">
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <Card><CardHeader><CardTitle>Published Payroll Schedule</CardTitle></CardHeader><CardContent className="space-y-3">{isLoading ? <p className="text-slate-400">Loading payroll dates…</p> : upcoming.length === 0 ? <p className="text-slate-400">No payroll periods have been published.</p> : upcoming.map(period => <div key={period.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-700 bg-slate-900/30 p-4"><div><div className="flex items-center gap-2"><p className="font-bold">{period.period_name}</p><Badge variant="outline">{period.status || 'upcoming'}</Badge></div><p className="mt-1 text-sm text-slate-400">{format(parseISO(period.start_date),'MMM d')} – {format(parseISO(period.end_date),'MMM d, yyyy')}</p><p className="text-sm font-semibold text-emerald-400">Deposit: {format(parseISO(period.deposit_date),'EEEE, MMM d, yyyy')}</p></div>{canManage && <div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => openForm(period)}><Pencil className="h-4 w-4" /></Button><Button size="sm" variant="outline" className="text-red-400" onClick={() => await confirmInApp('Delete this payroll period?') && deleteMutation.mutate(period.id)}><Trash2 className="h-4 w-4" /></Button></div>}</div>)}</CardContent></Card>
    </div>
