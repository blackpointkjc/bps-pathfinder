...
export default function AdminQRReports() {
  ...
  const getOfficerDisplay = (email, fallbackName) => {
    ...
    if (officer) return [officer.rank, officer.last_name].filter(Boolean).join(' ') || officer.full_name || fallbackName;
    return fallbackName || email;
  };

  // Primary source: live QRScanEvents — poll every 30 seconds
  const { data: allScans = [], dataUpdatedAt } = useQuery({
    queryKey: ['adminLiveQRScans', filterDate],
    queryFn: async () => {
      const params = {};
      if (filterDate) params.scanned_date = filterDate;
  ...
  // Fetch active checkpoints and property duty rules so this report follows QR Manager.
  ...
  // Real-time push: append new scans to cache instead of triggering a full refetch
  useEffect(() => {
    if (user?.role !== 'admin') return;
    const key = ['adminLiveQRScans', filterDate];
    const unsub = base44.entities.QRScanEvent.subscribe((event) => {
      if (event.type === 'create' && event.data?.scanned_date === filterDate) {
  ...
  // Enrich each group with checkpoint completion data
  const groups = Object.values(groupMap).map(g => {
    const rule = dutyRules.find(item => item.active !== false && item.property_site === g.property_site);
    const explicitIds = new Set((rule?.required_checkpoint_ids || []).map(String));
    const siteCheckpoints = allCheckpoints.filter(cp => cp.property_site === g.property_site && (explicitIds.size ? explicitIds.has(String(cp.id)) : cp.is_required !== false));
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <Select value={filterOfficer} onValueChange={setFilterOfficer}>
          ...
          <SelectContent>
            ...
            {allOfficers.map(o => <SelectItem key={o.email} value={o.email}>{getOfficerDisplay(o.email, o.name)}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
      ...
      <div className="space-y-3">
        {filtered.length === 0 && (
          <div className="text-center py-12 text-slate-400">
            ...
            <p className="text-sm mt-1">Scans appear here within 10 seconds of being logged.</p>
          </div>
        )}
        {filtered.map(g => (
          <Card
            ...
            style={{ borderLeftColor: g.missedCheckpoints.length > 0 ? '#ef4444' : '#22c55e' }}
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span className="font-semibold text-slate-900">{getOfficerDisplay(g.officer_email, g.officer_name)}</span>
                    <Badge className="bg-blue-100 text-blue-800 text-xs">{g.property_site}</Badge>
                    {g.totalRequired > 0 ? (
                      g.missedCheckpoints.length === 0
                        ? <Badge className="bg-green-100 text-green-800 text-xs">✓ All checkpoints scanned</Badge>
                        : <Badge className="bg-red-100 text-red-800 text-xs">⚠ {g.missedCheckpoints.length} missing</Badge>
                    ) : (
                      <Badge className="bg-slate-100 text-slate-600 text-xs">No required checkpoints set</Badge>
                    )}
                  </div>
                  ...
                  {/* Show missing checkpoint names inline */}
                  {g.missedCheckpoints.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1">
                      {g.missedCheckpoints.slice(0, 4).map(cp => (
                        <span key={cp.id} className="text-[10px] bg-red-50 text-red-700 border border-red-200 px-2 py-0.5 rounded-full flex items-center gap-1">
                          <XCircle className="w-2.5 h-2.5" /> {cp.checkpoint_name}
                        </span>
                      ))}
                      {g.missedCheckpoints.length > 4 && (
                        <span className="text-[10px] text-slate-500">+{g.missedCheckpoints.length - 4} more</span>
                      )}
                    </div>
                  )}
                </div>
          ...
          <DialogContent className="max-h-[90dvh] w-[calc(100vw-1rem)] max-w-2xl overflow-y-auto p-3 sm:p-6">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                ...
                {getOfficerDisplay(viewGroup.officer_email, viewGroup.officer_name)} — {viewGroup.property_site}
              </DialogTitle>
            </DialogHeader>
            <ScanDetailView group={viewGroup} allCheckpoints={allCheckpoints} filterDate={filterDate} />
          </DialogContent>
        </Dialog>
      )}
    </div>
...
function ScanDetailView({ group, allCheckpoints }) {
  ...
  // Build hourly rounds: find the earliest scan, align to hour, step through each hour
  const hourlyRounds = React.useMemo(() => {
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="bg-amber-50 rounded-lg p-3 text-center border border-amber-200">
          <p className="text-2xl font-bold text-amber-700">{group.scans.filter(s => s.scan_status === 'duplicate').length}</p>
