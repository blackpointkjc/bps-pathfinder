...
export default function DutySupervisorScheduling() {
  ...
  const toggleSupervisor = email => setForm(current => {
    ...
    return { ...current, supervisor_emails: next };
  });
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const writeAssignment = async (supervisor_email, location, id = null) => {
        const supervisor = users.find(person => lower(person.email) === lower(supervisor_email));
        if (!supervisor) throw new Error('Selected duty supervisor was not found.');
        const payload = {
          ...
          supervisor_name: [supervisor.rank, supervisor.last_name || supervisor.first_name].filter(Boolean).join(' '),
          supervisor_rank: supervisor.rank || '',
          status: 'scheduled',
          notes: form.notes || '',
      ...
      if (editing) {
        ...
        for (const location of requestedAreas) {
          ...
          await writeAssignment(supervisor_email, location, existing?.id || null);
        }
        for (const row of (editing.rows || []).filter(row => !requestedAreas.some(area => lower(area) === lower(row.location || 'ALL')))) await deleteAssignment(row);
      } else {
        for (const supervisor_email of requestedSupervisors) {
          for (const location of requestedAreas) await writeAssignment(supervisor_email, location);
        }
      }
  ...
  return <div className="bps-command-page min-h-full bg-[#080d16] p-4 text-white md:p-6">
    <div className="mx-auto max-w-[1700px] space-y-5">
      <section className="bps-command-hero rounded-[28px] border border-slate-700/80 bg-[#0d1420] p-5 shadow-2xl md:p-7">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between"><div><div className="text-[11px] font-black uppercase tracking-[.26em] text-cyan-300">Scheduling Command</div><h1 className="mt-2 text-3xl font-black md:text-4xl">Duty Supervisor Schedule</h1><p className="mt-2 max-w-3xl text-sm text-slate-400">Schedule command coverage by date, time, and site. Officers see the assigned duty supervisor directly on their published schedule.</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={()=>setDayOffset(v=>v-7)}><ChevronLeft className="h-4 w-4"/></Button><Button variant="outline" onClick={()=>setDayOffset(0)}>TODAY</Button><Button variant="outline" onClick={()=>setDayOffset(v=>v+7)}><ChevronRight className="h-4 w-4"/></Button><Button onClick={printSchedule} className="bg-cyan-700 hover:bg-cyan-600"><Printer className="mr-2 h-4 w-4"/>PRINT</Button></div></div>
      </section>
      ...
      <div className="grid gap-5 xl:grid-cols-[390px_minmax(0,1fr)]">
        <section className="rounded-[26px] border border-slate-700/80 bg-[#0d1420] p-5 shadow-xl">
          ...
          <div className="mt-5 space-y-4"><div><Label>Date</Label><Input type="date" value={form.assignment_date} onChange={e=>setForm({...form,assignment_date:e.target.value})}/></div><div className="grid grid-cols-2 gap-3"><div><Label>Start</Label><Input type="time" value={form.start_time} onChange={e=>setForm({...form,start_time:e.target.value})}/></div><div><Label>End</Label><Input type="time" value={form.end_time} onChange={e=>setForm({...form,end_time:e.target.value})}/></div></div><div><Label>Duty Supervisors</Label><p className="mb-2 mt-1 text-[11px] leading-4 text-slate-500">Select one or more supervisors. Multiple duty supervisors may cover the same property and time block.</p><div className="max-h-52 space-y-2 overflow-y-auto rounded-xl border border-slate-700 bg-[#09111d] p-2">{supervisors.map(person=>{const email=person.email;const active=form.supervisor_emails.includes(email);return <button type="button" key={person.id||email} onClick={()=>toggleSupervisor(email)} className={`flex w-full items-center justify-between rounded-lg border px-3 py-2 text-left text-xs font-bold ${active?'border-emerald-500/50 bg-emerald-500/10 text-emerald-100':'border-slate-700 text-slate-300 hover:border-slate-500'}`}><span>{[person.rank,person.last_name||person.first_name].filter(Boolean).join(' ')}</span>{active&&<Check className="h-4 w-4"/>}</button>})}</div><div className="mt-2 text-[10px] font-bold text-emerald-300">SELECTED: {form.supervisor_emails.length} SUPERVISOR{form.supervisor_emails.length===1?'':'S'}</div></div><div><Label>Coverage Areas</Label><p className="mb-2 mt-1 text-[11px] leading-4 text-slate-500">Choose one or more areas. The same duty supervisor can cover multiple sites during the same time block.</p><div className="max-h-56 space-y-2 overflow-y-auto rounded-xl border border-slate-700 bg-[#09111d] p-2"><button type="button" onClick={()=>toggleCoverageArea('ALL')} className={`flex w-full items-center justify-between rounded-lg border px-3 py-2 text-left text-xs font-bold ${form.locations.includes('ALL')?'border-cyan-500/50 bg-cyan-500/10 text-cyan-100':'border-slate-700 text-slate-300'}`}><span>All Sites / Company Duty</span>{form.locations.includes('ALL')&&<Check className="h-4 w-4"/>}</button>{locations.map(location=>{const active=form.locations.includes(location.site_name);return <button type="button" key={location.id} onClick={()=>toggleCoverageArea(location.site_name)} className={`flex w-full items-center justify-between rounded-lg border px-3 py-2 text-left text-xs font-bold ${active?'border-cyan-500/50 bg-cyan-500/10 text-cyan-100':'border-slate-700 text-slate-300 hover:border-slate-500'}`}><span>{location.site_name}</span>{active&&<Check className="h-4 w-4"/>}</button>})}</div><div className="mt-2 text-[10px] font-bold text-cyan-300">SELECTED: {form.locations.join(' • ')}</div></div><div><Label>Notes</Label><Input value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} placeholder="Optional duty notes"/></div>{canManage&&<div className="flex gap-2"><Button className="flex-1 bg-cyan-700 hover:bg-cyan-600" onClick={save} disabled={saving}><CheckCircle2 className="mr-2 h-4 w-4"/>{saving?'SAVING…':editing?'UPDATE COVERAGE':'SCHEDULE SUPERVISOR'}</Button>{editing&&<Button variant="outline" onClick={()=>reset(form.assignment_date)}>CANCEL</Button>}</div>}</div>
        </section>
        <section className="rounded-[26px] border border-slate-700/80 bg-[#0d1420] p-5 shadow-xl"><div className="flex items-center justify-between gap-3"><div><div className="text-xs font-black uppercase tracking-[.18em] text-amber-300">7-Day Command Coverage</div><h2 className="mt-1 text-2xl font-black">{format(windowStart,'MMM d')} – {format(addDays(windowStart,6),'MMM d, yyyy')}</h2></div><Badge variant="outline">{groupedVisible.length} COVERAGE BLOCKS</Badge></div><div className="mt-5 grid gap-3 md:grid-cols-2 2xl:grid-cols-3">{dates.map(date=>{const day=groupedVisible.filter(row=>row.assignment_date===date);return <div key={date} className="overflow-hidden rounded-2xl border border-slate-700 bg-[#101826]"><div className="border-b border-slate-700 px-4 py-3"><div className="text-sm font-black">{format(new Date(`${date}T12:00:00`),'EEEE')}</div><div className="text-xs text-slate-500">{format(new Date(`${date}T12:00:00`),'MMM d')}</div></div><div className="space-y-2 p-3">{day.map(group=><button type="button" key={`${group.assignment_date}-${group.start_time}-${group.end_time}-${group.supervisor_email}`} onClick={()=>canManage&&startEdit(group)} className="w-full rounded-xl border border-slate-700 bg-[#0a111d] p-3 text-left transition hover:border-cyan-600/60"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><div className="truncate text-sm font-black text-white">{group.supervisor_name||personLabel(group.supervisor_email)}</div><div className="mt-1 flex items-center gap-1 text-xs text-cyan-300"><CalendarClock className="h-3.5 w-3.5"/>{group.start_time}–{group.end_time}</div><div className="mt-2 flex items-start gap-1 text-xs text-slate-400"><MapPin className="mt-0.5 h-3.5 w-3.5 shrink-0"/><span className="break-words">{group.locations.map(location=>location==='ALL'?'All Sites':location).join(' • ')}</span></div>{group.notes&&<div className="mt-2 text-[10px] leading-4 text-slate-500">{group.notes}</div>}</div>{canManage&&<span onClick={e=>{e.stopPropagation();remove(group);}} className="rounded-lg border border-red-800/60 p-2 text-red-300 hover:bg-red-950/30"><Trash2 className="h-3.5 w-3.5"/></span>}</div></button>)}{!day.length&&<button type="button" onClick={()=>{reset(date);setForm(current=>({...current,assignment_date:date}));}} className="w-full rounded-xl border border-dashed border-slate-700 px-3 py-8 text-center text-xs font-bold text-slate-600 hover:border-cyan-700 hover:text-cyan-400">NO DUTY COVERAGE</button>}</div></div>})}</div></section>
      </div>
    </div>
  </div>;
}
