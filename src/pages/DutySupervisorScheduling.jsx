...
import { ShieldCheck, ChevronLeft, ChevronRight, CalendarClock, MapPin, Trash2, CheckCircle2 } from 'lucide-react';
...
export default function DutySupervisorScheduling() {
  ...
  const [form,setForm] = useState({ assignment_date:format(new Date(),'yyyy-MM-dd'), start_time:'18:00', end_time:'06:00', location:'ALL', supervisor_email:'', notes:'' });
  ...
  const reset = date => { setEditing(null); setForm({ assignment_date:date||format(windowStart,'yyyy-MM-dd'), start_time:'18:00', end_time:'06:00', location:'ALL', supervisor_email:'', notes:'' }); };
  const startEdit = row => { setEditing(row); setForm({ assignment_date:row.assignment_date,start_time:row.start_time,end_time:row.end_time,location:row.location||'ALL',supervisor_email:row.supervisor_email,notes:row.notes||'' }); };
  const save = async () => {
    if(!canManage) return;
    if(!form.supervisor_email) return toast.error('Select a duty supervisor.');
    const conflict=assignments.find(row=>row.id!==editing?.id&&row.assignment_date===form.assignment_date&&lower(row.status)!=='cancelled'&&overlaps(row.start_time,row.end_time,form.start_time,form.end_time)&&((row.location||'ALL')==='ALL'||form.location==='ALL'||lower(row.location)===lower(form.location)));
    if(conflict) return toast.error(`Duty coverage overlaps ${conflict.supervisor_name||personLabel(conflict.supervisor_email)} from ${conflict.start_time}-${conflict.end_time}.`);
    setSaving(true);
    try {
      const response=await base44.functions.invoke('manageDutySupervisorSchedule',{ action:'save', assignment:{...form,id:editing?.id||'',status:'scheduled'} });
      ...
      toast.success(editing?'Duty supervisor assignment updated.':'Duty supervisor scheduled.'); reset(form.assignment_date);
