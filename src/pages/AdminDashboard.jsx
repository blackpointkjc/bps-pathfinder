      ...
      <div className="mx-auto max-w-[1500px] space-y-5 sm:space-y-6">
        ...
        <section className="rounded-2xl border border-amber-500/20 bg-[#0a1421] p-5 shadow-lg">
          ...
          <div className="mt-4 grid gap-2 xl:grid-cols-2">
            ...
            {!!adminWork.recently_completed?.length && <details className="mt-3 rounded-xl border border-slate-800 bg-slate-950/30 p-3 xl:col-span-2"><summary className="cursor-pointer text-xs font-black uppercase tracking-wide text-slate-400">Recently completed ({adminWork.recently_completed.length})</summary><div className="mt-3 grid gap-2 sm:grid-cols-2">{adminWork.recently_completed.slice(0,8).map(item => <div key={item.id} className="flex flex-wrap items-start justify-between gap-2 rounded-lg border border-slate-800 px-3 py-2 text-xs"><div><div className="font-bold text-slate-300">{item.title || item.task_key}</div>{item.person && <div className="mt-0.5 text-cyan-200">{item.person}</div>}{item.completion_note && <div className="mt-1 text-slate-500">{item.completion_note}</div>}</div><span className="text-emerald-300">{item.completed_at ? new Date(item.completed_at).toLocaleString() : 'Completed'}</span></div>)}</div></details>}
