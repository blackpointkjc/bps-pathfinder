...
const actions = [
  { label: 'Company Employees', detail: 'Employee records, assignments and status', icon: Users, section: 'employees', tool: 'employees' },
  { label: 'Time Entries', detail: 'Review and manage employee time records', icon: Clock3, section: 'support', tool: 'timeentries' },
  { label: 'PTO & Leave', detail: 'Approvals, manual PTO and leave history', icon: CalendarClock, section: 'leave', tool: 'pto' },
  { label: 'Performance Reviews', detail: 'HR performance review workflow', icon: ClipboardCheck, section: 'clients', tool: 'reviews' },
  { label: 'Client Assignments', detail: 'Manage client accounts and employee assignments', icon: Building2, section: 'employees', tool: 'clients' },
];

const hrCenterUrl = (section, tool, extra = {}) => {
  const params = new URLSearchParams({ section, tool });
  Object.entries(extra).forEach(([key, value]) => {
    if (value !== undefined && value !== null && String(value)) params.set(key, String(value));
  });
  return `${createPageUrl('HRCenter')}?${params.toString()}`;
};

const workQueueTaskUrl = item => {
  const destination = item?.page === 'AdminPTOApproval'
    ? { section: 'leave', tool: 'pto' }
    : item?.page === 'AdminPerformanceReviews'
      ? { section: 'clients', tool: 'reviews' }
      : { section: 'support', tool: 'timeentries' };
  const extra = {
    queue_task: item?.id,
    queue_kind: item?.kind,
    record_id: item?.page !== 'ManageTimeEntries' ? item?.source_id : '',
    entry_id: item?.kind === 'late_clock_out' ? item?.source_id : '',
  };
  return hrCenterUrl(destination.section, destination.tool, extra);
};
      ...
      <div className="mx-auto max-w-[1500px] space-y-5">
        <section className="relative overflow-hidden rounded-3xl border border-cyan-500/20 bg-gradient-to-br from-[#102239] via-[#0b1726] to-[#07101c] p-6 shadow-2xl md:p-8">
          ...
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2"><div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-sm font-black text-emerald-200">{isLoading ? 'LOADING HR DATA' : `${pendingActions.length} ACTIONS WAITING`}</div><button type="button" onClick={refreshAll} disabled={isFetching || workforceFetching} className="rounded-2xl border border-cyan-500/30 bg-cyan-500/10 px-4 py-3 text-xs font-black text-cyan-200 hover:bg-cyan-500/20 disabled:opacity-50">{isFetching || workforceFetching ? 'CHECKING…' : 'REFRESH TASKS'}</button></div>
          </div>
        </section>

        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {[['Active Employees', counts.active_employees ?? employees.length, Users, 'Directory accounts currently active'], ['Clocked In', counts.clocked_in ?? activeEntries.length, UserCheck, 'Employees with open time entries'], ['Attendance Exceptions', (counts.missed_clock_ins || 0) + (counts.late_clock_outs || 0), AlertCircle, 'Missed clock-ins and late clock-outs awaiting review'], ['All Pending HR Work', counts.total || 0, ClipboardCheck, 'Attendance, PTO and performance reviews']].map(([label,value,Icon,detail]) => <div key={label} className="bps-kpi-card rounded-2xl border border-slate-800 bg-[#0b1624] p-5 shadow-lg"><div className="bps-kpi-icon bg-cyan-500/10 text-cyan-300"><Icon className="h-5 w-5"/></div><span className="bps-kpi-value text-3xl font-black">{value}</span><div className="bps-kpi-label text-sm font-black">{label}</div><div className="bps-kpi-detail text-xs text-slate-500">{detail}</div></div>)}
        </div>
