...
import { BarChart3, MapPin, Printer, QrCode } from 'lucide-react';
...
const tools = [
  { id: 'checkpoints', label: 'Checkpoint Management', icon: MapPin },
  { id: 'print', label: 'Print Manager', icon: Printer },
  { id: 'reports', label: 'Patrol Reports', icon: BarChart3 },
];

export default function AdminQRCenter() {
  const [tool, setTool] = useState('checkpoints');
  const Active = tool === 'print' ? AdminQRPrintManager : tool === 'reports' ? AdminQRReports : AdminQRCheckpoints;
  return (
      ...
      <div className="mx-auto max-w-[1700px] space-y-5">
        <section className="rounded-[28px] border border-slate-700/80 bg-[#0d1420] p-5 shadow-2xl md:p-7">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div><div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[.24em] text-cyan-300"><QrCode className="h-4 w-4"/>QR Patrol Command</div><h1 className="mt-2 text-3xl font-black md:text-4xl">QR Patrol Management</h1><p className="mt-2 max-w-3xl text-sm text-slate-400">Manage checkpoints, duty rules, printable QR labels, and patrol compliance reports from one page.</p></div>
