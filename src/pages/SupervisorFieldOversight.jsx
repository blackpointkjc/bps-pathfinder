  ...
  return <div className="min-h-screen bg-[#07111d] text-slate-100">
    ...
    <main className="mx-auto max-w-[1700px] space-y-5 p-4 md:p-6">
      ...
      <section className="grid gap-4 xl:grid-cols-[1.15fr_.85fr]">
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-700 bg-[#0b1725]"><div className="border-b border-slate-700 px-4 py-3"><h2 className="font-black">Supervisor Attention</h2><p className="text-xs text-slate-500">Acknowledgement and welfare conditions only</p></div>{welfareLoading?<div className="p-8 text-center text-sm text-slate-500">Loading welfare status…</div>:welfareError?<div className="p-4 text-sm text-red-300">{welfareError.message}</div>:attention.length===0?<div className="p-8 text-center"><CheckCircle2 className="mx-auto mb-2 h-9 w-9 text-emerald-400"/><div className="font-bold text-emerald-300">No overdue welfare conditions</div><div className="mt-1 text-xs text-slate-500">{pendingAck.length} assignment{pendingAck.length===1?'':'s'} currently awaiting acknowledgement.</div></div>:<div className="divide-y divide-slate-800">{attention.map(row=><div key={row.assignment_id} className="bg-red-950/15 p-4"><div className="flex flex-wrap items-center gap-2"><Badge className="bg-red-700">ATTENTION</Badge><span className="text-xs font-black text-cyan-300">CAD {row.cad_number}</span></div><div className="mt-2 font-black">{row.unit_number?`Unit ${row.unit_number} · `:''}{row.officer_name}</div><div className="mt-1 text-xs text-slate-400">{row.incident} · {row.location}</div><div className="mt-2 grid grid-cols-2 gap-2"><Info label="Timer" value={elapsed(row.elapsed_seconds)} alert/><Info label="GPS" value={gpsAge(row.gps_updated_at)} alert={!row.gps_updated_at}/></div><div className="mt-3 flex gap-2"><Button size="sm" variant="outline" onClick={()=>openCad(row)} className="flex-1">View CAD</Button><Button size="sm" onClick={()=>escalate(row)} disabled={!!workingId} className="flex-1 bg-red-700 hover:bg-red-600">Escalate Welfare</Button></div></div>)}</div>}</div>
      </section>
    </main>
  </div>;
}
