...
export default function ShiftHandover(){
  ...
  const mySites=useMemo(()=>[...new Set(schedules.filter(s=>s.officer_email===user?.email).map(s=>s.location).filter(Boolean))].sort(),[schedules,user?.email]);
  ...
  return <div className="min-h-screen p-4 md:p-8"><div className="mx-auto max-w-5xl space-y-6">
    ...
    <div className="space-y-3">{visible.length===0?<Card><CardContent className="p-8 text-center text-slate-400">No handovers for your assigned sites.</CardContent></Card>:visible.map(h=><Card key={h.id} className={h.incoming_officer_email===user?.email&&!h.acknowledged_by_incoming?'border-amber-600':''}><CardHeader><div className="flex items-start justify-between gap-3"><div><CardTitle className="flex items-center gap-2"><MapPin className="h-4 w-4"/>{h.location}</CardTitle><p className="mt-1 text-sm text-slate-400">{h.departing_officer_name} → {h.incoming_officer_name||'Next assigned officer'} • {h.shift_date}</p></div>{h.acknowledged_by_incoming?<Badge className="bg-emerald-700">Acknowledged</Badge>:h.incoming_officer_email===user?.email?<Button size="sm" onClick={()=>acknowledge.mutate(h.id)}><CheckCircle className="mr-2 h-4 w-4"/>Acknowledge</Button>:<Badge variant="outline">Pending</Badge>}</div></CardHeader><CardContent className="space-y-3 text-sm"><div><strong>Key updates:</strong><p className="text-slate-300">{h.key_updates}</p></div>{h.ongoing_issues&&<div><strong>Ongoing issues:</strong><p className="text-slate-300">{h.ongoing_issues}</p></div>}{h.pending_tasks&&<div><strong>Pending tasks:</strong><p className="text-slate-300">{h.pending_tasks}</p></div>}{h.equipment_status&&<div><strong>Equipment status:</strong><p className="text-slate-300">{h.equipment_status}</p></div>}</CardContent></Card>)}</div>
  </div></div>;
}