...
export default function Navigation() {
    ...
    return (
        <div className="relative h-full min-h-0 w-full min-w-0 overflow-hidden bg-[#07101a]">
            ...
            {isNavigating && navDestination && (
                <div className="bps-navigation-drive-hud pointer-events-none absolute bottom-0 left-0 right-0 z-[1210] px-3">
                    <div className="mx-auto flex w-full max-w-[680px] items-end gap-2">
                        <button type="button" onClick={() => setShowAddressSearch(true)} className="pointer-events-auto flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-slate-300 bg-white text-blue-700 shadow-[0_16px_40px_rgba(0,0,0,.35)]" title="Route options">
                            <Navigation2 className="h-6 w-6" />
                        </button>
                        <div className="pointer-events-auto min-w-0 flex-1 overflow-hidden rounded-2xl border border-slate-300 bg-white text-slate-950 shadow-[0_16px_40px_rgba(0,0,0,.38)]">
                            <div className="flex items-center justify-between gap-3 border-b border-slate-200 px-4 py-2">
                                <div className="min-w-0">
                                    <div className="truncate text-sm font-black text-blue-700">{navDestination.name}</div>
                                    <div className="truncate text-[11px] font-semibold text-slate-500">{formatInstruction(activeNavStep)}</div>
                                </div>
                                <div className="shrink-0 text-right">
                                    <div className="text-2xl font-black leading-none text-slate-950">{navDurationMinutes}<span className="ml-1 text-sm font-black">min</span></div>
