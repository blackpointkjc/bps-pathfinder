...
export default function OfficerInbox() {
  ...
  const conversations = React.useMemo(() => {
    ...
    allMessages.forEach(msg => {
      ...
      if (!otherParty || (msg.recipients?.length > 1 && !isSender)) return; // Skip group messages for now
  ...
  const sendMessageMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['myDirectMessages'] });
      setNewMessage("");
    },
  });
      ...
      <div className={`w-full md:w-80 border-r border-slate-200 flex flex-col bg-slate-50 ${selectedConversation || showNewChat ? 'hidden md:flex' : 'flex'}`}>
        <div className="p-4 border-b border-slate-200 bg-white">
          <div className="flex items-center justify-between mb-3">
            <h1 className="text-xl font-bold text-slate-900">Private Messages</h1>
            {totalUnread > 0 && (
              <Badge className="bg-blue-500 text-white">{totalUnread}</Badge>
            )}
          </div>
          <Button 
        ...
        {showNewChat ? (
                ...
                {filteredUsers.map(u => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500">{u.rank || 'Officer'}</p>
                    </div>
                    {u.role === 'admin' && <Badge className="bg-amber-100 text-amber-800 text-xs ml-auto">Admin</Badge>}
                  </div>
                ))}
              </ScrollArea>
            )}
          ...
          <>
            ...
            <div className="p-4 border-b border-slate-200 bg-white flex items-center gap-3">
              ...
              <div>
                ...
                {getPartnerInfo(selectedConversation).isAdmin && (
          ...
          <div className="flex-1 flex items-center justify-center bg-slate-50">
            <div className="text-center">
              ...
              <h3 className="text-xl font-semibold text-slate-700">Private Messages</h3>
