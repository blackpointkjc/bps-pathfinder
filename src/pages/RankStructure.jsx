...
import { Shield, GitBranch, Mail, Phone, X } from 'lucide-react';
...
function PersonCard({ person, users, onOpen }) {
  const supervisor = users.find(u=>u.id===person.supervisor_id);
  const isTop = person.rank === 'Colonel';
  const nearestHigherRank = (() => { const i=rankIndex(person.rank); if(i<=0||i>=99)return null; for(let x=i-1;x>=0;x--){if(users.some(u=>rankIndex(u.rank)===x))return RANKS[x];} return null; })();
  const reportsToName = supervisor ? displayName(supervisor) : isTop ? 'Top of Command' : 'Awaiting assignment';
  const reportsToRank = supervisor?.rank || (isTop ? 'Top of Command' : nearestHigherRank || 'No higher rank available');
  return <button type="button" onClick={()=>onOpen?.(person)} className="w-full rounded-xl border border-slate-700 bg-slate-950/70 p-3 text-left transition hover:border-blue-500/70 hover:bg-slate-950 focus:outline-none focus:ring-2 focus:ring-blue-500/50">
...
function TreeNode({ person, branch, allUsers, onOpen, depth = 0 }) {
  ...
  return <div className={depth ? 'ml-4 border-l border-slate-700 pl-4 sm:ml-6 sm:pl-5' : ''}>
    ...
    {children.length > 0 && <div className="mt-2 space-y-2">{children.map(child=><TreeNode key={child.id} person={child} branch={branch} allUsers={allUsers} onOpen={onOpen} depth={depth+1}/>)}</div>}
  </div>;
}
...
function PlatoonBranch({ letter, users, allUsers, onOpen }) {
  ...
  return <section className="overflow-hidden rounded-2xl border border-slate-700 bg-slate-900">
    ...
    <div className="space-y-4 p-4">{roots.map(person=><TreeNode key={person.id} person={person} branch={branch} allUsers={allUsers} onOpen={onOpen}/>)}{branch.length===0&&<div className="py-10 text-center text-sm text-slate-600">No personnel assigned to Platoon {letter}.</div>}</div>
  </section>;
}
...
export default function RankStructure(){
  ...
  return <div className="min-h-screen bg-slate-950 p-4 text-white md:p-7"><div className="mx-auto max-w-7xl space-y-5">
    ...
    <section className="rounded-2xl border border-amber-500/30 bg-amber-950/10 p-4"><div className="mb-4 text-center"><div className="text-xs font-black uppercase tracking-[.25em] text-amber-300">Shared Command</div><div className="mt-1 text-sm text-slate-400">Colonel · Lt Colonel · Major — oversight of Platoon A and Platoon B</div></div><div className="mx-auto grid max-w-4xl gap-3 md:grid-cols-3">{command.map(person=><PersonCard key={person.id} person={{...person,platoon:'Command'}} users={active} onOpen={setSelectedPerson}/>)}</div>{!command.length&&<div className="text-center text-sm text-slate-600">No shared-command personnel assigned.</div>}</section>
