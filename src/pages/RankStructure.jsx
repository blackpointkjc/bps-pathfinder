...
export default function RankStructure(){
  ...
  return <div className="min-h-screen bg-slate-950 p-4 text-white md:p-7"><div className="mx-auto max-w-7xl space-y-5">
    ...
    <div className="grid gap-4 lg:grid-cols-2">{RANKS.map(rank=>{const list=active.filter(u=>u.rank===rank);return <section key={rank} className="overflow-hidden rounded-xl border border-slate-800 bg-slate-900"><div className="flex items-center border-b border-slate-800 bg-slate-900/90 px-4 py-3"><div><div className="text-sm font-black text-amber-300">{rank.toUpperCase()}</div><div className="text-[10px] text-slate-500">{list.length} PERSONNEL</div></div></div><div className="divide-y divide-slate-800">{list.length===0?<div className="p-4 text-xs text-slate-600">No active personnel assigned.</div>:list.map(o=><div key={o.id} className="flex items-center gap-3 p-3 hover:bg-slate-800/50">{o.profile_photo_url?<img src={o.profile_photo_url} className="h-10 w-10 rounded-full object-cover"/>:<div className="flex h-10 w-10 items-center justify-center rounded-full bg-slate-800 text-xs font-black">{o.first_name?.[0]}{o.last_name?.[0]}</div>}<div className="min-w-0 flex-1"><div className="font-bold">{o.first_name} {o.last_name}</div><div className="text-xs text-slate-500">{o.division||'No division assigned'}</div></div>{o.unit_number&&<Badge className="bg-blue-950 text-blue-300">Officer #{o.unit_number}</Badge>}</div>)}</div></section>})}</div>
