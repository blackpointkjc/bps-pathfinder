...
export default function MyPerformanceAnalytics() {
  ...
  const performanceFactors = useMemo(() => {
    ...
    if (supervisorRatingStats.count > 0) {
      factors.push({ metric: 'Supervisor Rating', value: `${supervisorRatingStats.score}%`, severity: supervisorRatingStats.score >= 80 ? 'positive' : 'negative', reason: `${supervisorRatingStats.count} supervisor performance review${supervisorRatingStats.count === 1 ? '' : 's'} average ${supervisorRatingStats.avgRating.toFixed(1)} of 5.`, details: supervisorRatingStats.items.map(item => `${item.review_date}: ${Number(item.effective_rating ?? item.final_rating ?? item.overall_rating).toFixed(1)}/5${item.reviewer_name ? ` by ${item.reviewer_name}` : ''}`) });
    }
