  ...
  return (
    <div className="min-h-[calc(100vh-190px)] bg-[#070d17] p-4 text-white md:p-6">
      <div className="mx-auto max-w-[1500px] space-y-5">
        ...
        <div className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]"><section className="rounded-2xl border border-slate-800 bg-[#0a1421] p-5"><div className="text-xs font-black uppercase tracking-[.16em] text-emerald-300">Payroll Activity</div><h3 className="mt-1 text-xl font-black">Recent payroll entries</h3><div className="mt-4 space-y-2">{recentPayroll.length ? recentPayroll.map((row,index) => <div key={row.id || index} className="flex items-center justify-between gap-4 rounded-xl border border-slate-800 bg-[#0d1a2a] px-4 py-3"><div><div className="text-sm font-bold">{row.officer_name || row.employee_name || row.officer_email || 'Employee'}</div><div className="text-xs text-slate-500">{row.period_name || currentPeriod?.period_name || 'Payroll entry'}</div></div><div className="text-sm font-black text-emerald-200">${Number(row.total_payment_due || row.net_pay || 0).toLocaleString(undefined,{maximumFractionDigits:2})}</div></div>) : <div className="rounded-xl border border-dashed border-slate-700 p-6 text-center text-sm text-slate-500">No payroll entries loaded.</div>}</div></section><section className="rounded-2xl border border-slate-800 bg-[#0a1421] p-5"><div className="text-xs font-black uppercase tracking-[.16em] text-emerald-300">Expense Queue</div><h3 className="mt-1 text-xl font-black">Awaiting review</h3><div className="mt-4 space-y-2">{pendingExpenses.slice(0,5).map((row,index) => <div key={row.id || index} className="rounded-xl border border-slate-800 bg-[#0d1a2a] px-4 py-3"><div className="flex items-center justify-between gap-3"><div className="text-sm font-bold">{row.officer_name || row.officer_email || 'Expense'}</div><div className="text-sm font-black text-amber-200">${Number(row.amount || 0).toLocaleString(undefined,{maximumFractionDigits:2})}</div></div><div className="mt-1 text-xs text-slate-500">{row.expense_type || row.category || 'Reimbursement'}</div></div>)}{!pendingExpenses.length && <div className="rounded-xl border border-dashed border-slate-700 p-6 text-center text-sm text-slate-500">No pending expenses.</div>}</div></section></div>
