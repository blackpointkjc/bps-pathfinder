...
const payrollDateLabel = row => {
  const start = formatPayrollDate(row.pay_period_start);
  const end = formatPayrollDate(row.pay_period_end);
  const payDate = formatPayrollDate(row.pay_date);
  const period = start && end ? `Period ${start} – ${end}` : start ? `Period starting ${start}` : '';
  return [period, payDate ? `Pay date ${payDate}` : ''].filter(Boolean).join(' · ') || 'Payroll dates unavailable';
};

export default function AccountingOverview() {
  const { data: payload = {}, isLoading, isFetching, error, refetch } = useQuery({
    queryKey: ['accountingOverviewSnapshot'],
    queryFn: async () => {
      const result = await base44.functions.invoke('getAccountingData', { scope: 'overview' });
  ...
  const directory = buildDirectoryIndex(payload.users || []);
  const payrollEntries = payload.payrollEntries || payload.payroll_entries || [];
  const periods = payload.payrollPeriods || payload.payroll_periods || payload.periods || [];
  const expenses = payload.expenseReports || [];
  const loadErrors = payload.load_errors || [];
  ...
  return (
    <div className="min-h-[calc(100vh-190px)] bg-[#070d17] p-4 text-white md:p-6">
      <div className="mx-auto max-w-[1500px] space-y-5">
        <section className="relative overflow-hidden rounded-3xl border border-emerald-500/20 bg-gradient-to-br from-[#0f2b27] via-[#0b1724] to-[#070d17] p-6 shadow-2xl md:p-8"><div className="absolute -right-24 -top-24 h-80 w-80 rounded-full bg-emerald-500/10 blur-3xl"/><div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[.22em] text-emerald-300"><DollarSign className="h-4 w-4"/>Finance & Accounting</div><h2 className="mt-2 text-3xl font-black md:text-4xl">Financial Operations Dashboard</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">Payroll workload, current period, reimbursements and finance actions visible immediately.</p></div><div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-sm font-black text-emerald-200">{isLoading ? 'LOADING FINANCE DATA' : error ? 'FINANCE DATA ERROR' : loadErrors.length ? 'PARTIAL FINANCE DATA' : isFetching ? 'REFRESHING FINANCE DATA' : 'FINANCE ONLINE'}</div></div></section>

        {error && <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-red-500/40 bg-red-500/10 p-4 text-sm text-red-200"><span>Financial data could not be loaded: {error?.response?.data?.error || error.message}</span><button type="button" onClick={() => refetch()} className="rounded-lg border border-red-400/40 bg-red-500/10 px-3 py-2 text-xs font-black hover:bg-red-500/20">RETRY</button></div>}
