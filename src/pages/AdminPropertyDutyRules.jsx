...
export default function AdminPropertyDutyRules() {
  ...
  const dutyRoles = new Set((user?.additional_roles || []).map(role=>String(role).toLowerCase()));
  const canManageDutyRules = user?.role === 'admin' || dutyRoles.has('full_access');
  ...
  const {data:locations=[]} = useQuery({queryKey:['activeLocations','dutyRules'],queryFn:async()=>{try{const rows=await listDirectoryLocations('site_name',1000);if(Array.isArray(rows)&&rows.length)return rows.filter(x=>x.active!==false);}catch{} const direct=await base44.entities.Location.list('site_name',1000);return (direct||[]).filter(x=>x.active!==false);},enabled:canManageDutyRules,placeholderData:[],staleTime:10*60*1000,refetchOnWindowFocus:false});
  ...
  if(isLoadingAuth) return <div className="p-8 text-center text-slate-500">Loading duty-rule access…</div>;
  if(!canManageDutyRules) return <div className="p-8 text-center text-slate-500">Admin access required.</div>;
  const sites=(locations||[]).map(x=>x.site_name);
  return <div className="bps-command-page min-h-0 bg-[#07111d] p-3 text-white md:p-4"><div className="mx-auto max-w-7xl space-y-3">
    <section className="rounded-xl border border-slate-700 bg-[#0a1623] p-3 shadow-lg"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-lg border border-cyan-500/30 bg-cyan-500/10"><Settings2 className="h-4 w-4 text-cyan-300"/></div><div><div className="text-[9px] font-black uppercase tracking-[.16em] text-cyan-300">Patrol Standards</div><h2 className="mt-0.5 text-lg font-black">Property Duty Rules</h2><p className="mt-1 text-[11px] text-slate-400">DAR, incident-report, and QR compliance requirements by property.</p></div></div><Button type="button" onClick={openNewRule} className="shrink-0 bg-cyan-700 hover:bg-cyan-600"><Plus className="mr-2 h-4 w-4"/>New Property Rule</Button></div></section>
    ...
    <Dialog open={showForm} onOpenChange={setShowForm}><DialogContent className="max-h-[88vh] max-w-2xl overflow-y-auto border-slate-700 bg-slate-950 text-white"><DialogHeader><DialogTitle>{editingRule?'Property Duty Rules':'New Property Duty Rule'}{ruleForm.property_site?` — ${ruleForm.property_site}`:''}</DialogTitle></DialogHeader><div className="space-y-5">
      {!editingRule&&<div><Label>Property</Label><select value={ruleForm.property_site} onChange={e=>setRuleForm(p=>({...p,property_site:e.target.value,required_checkpoint_ids:[]}))} className="mt-2 h-10 w-full rounded-md border border-slate-700 bg-slate-900 px-3 text-sm text-white"><option value="">Select property...</option>{locations.filter(location=>!rules.some(rule=>rule.property_site===location.site_name)).map(location=><option key={location.id||location.site_name} value={location.site_name}>{location.site_name}</option>)}{locations.length>0&&locations.every(location=>rules.some(rule=>rule.property_site===location.site_name))&&<option value="" disabled>All active properties already have a rule</option>}</select></div>}
