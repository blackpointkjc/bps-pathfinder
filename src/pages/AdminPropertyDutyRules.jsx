...
export default function AdminPropertyDutyRules() {
  ...
  return <div className="bps-command-page min-h-0 bg-[#07111d] p-3 text-white md:p-4"><div className="mx-auto max-w-7xl space-y-3">
    ...
    <Dialog open={showForm} onOpenChange={setShowForm}><DialogContent className="max-h-[88vh] max-w-2xl overflow-y-auto border-slate-700 bg-slate-950 text-white"><DialogHeader><DialogTitle>{editingRule?'Property Duty Rules':'New Property Duty Rule'}{ruleForm.property_site?` — ${ruleForm.property_site}`:''}</DialogTitle></DialogHeader><div className="space-y-5">
      {!editingRule&&<div><Label>Property</Label><select value={ruleForm.property_site} onChange={e=>selectRuleSite(e.target.value)} className="mt-2 h-10 w-full rounded-md border border-slate-700 bg-slate-900 px-3 text-sm text-white"><option value="">Select property...</option>{locations.map(location=>{const configured=rules.some(rule=>rule.property_site===location.site_name);return <option key={location.id||location.site_name} value={location.site_name}>{location.site_name}{configured?' · configured':''}</option>})}</select>{locations.length===0&&<p className="mt-2 text-xs text-amber-300">No active properties were returned. Use Sites to verify location access.</p>}</div>}
