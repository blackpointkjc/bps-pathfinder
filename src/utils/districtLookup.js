 ...
 * Returns a human-readable string like "Richmond Beat 111A" or "Henrico - Brookland" or null.
 */
export async function lookupDistrict(lat, lng) {
    ...
    const [richmond, henrico, chesterfield] = await Promise.allSettled([
        fetch(`https://services1.arcgis.com/k3vhq11XkBNeeOfM/arcgis/rest/services/Police_Beats/FeatureServer/0/query?${qs}&outFields=Name`).then(r => r.json()),
        fetch(`https://portal.henrico.gov/mapping/rest/services/Layers/Magisterial_Districts_2021/MapServer/0/query?${qs}&outFields=MAG_DIST_NAME,NAME`).then(r => r.json()),
        fetch(`https://services3.arcgis.com/TsynfzBSE6sXfoLq/ArcGIS/rest/services/Administrative_ProdA/FeatureServer/9/query?${qs}&outFields=*`).then(r => r.json()),
    ]);
